...
//MessageTime wraps the time response
...
//BuisnessLogicGetTime does awesome BuisnessLogic
func BuisnessLogicGetTime() *MessageTime {
	mess := &MessageTime{}
	mess.Time = time.Now().Unix()
	return mess
}
...
func Time(res http.ResponseWriter, req *http.Request) {
	...
	res.Header().Add("Content-type", "application/json")
	pointless := BuisnessLogicGetTime()
	if err := jsonEncoder.Encode(pointless); err != nil { //encode our data and write it back to the response stream
		res.WriteHeader(http.StatusInternalServerError)
		return
	}
